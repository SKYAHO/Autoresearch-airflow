...
log에 남는다고 가정하지 않습니다. stdout/stderr는 화면에 표시하는 동시에 운영자
로컬의 임시 파일에 캡처합니다. 아래 캡처·검사·삭제 명령은 같은 로컬 shell session에서
순서대로 실행합니다.
...
합성 프로세스의 캡처에서 다음 두 성공 식별자를 각각 확인합니다.
...
정상 smoke에서는 다음 두 식별자가 각각 한 번 출력되어야 합니다.
...
성공 식별자가 없거나 메일이 오지 않으면 같은 캡처에서 SMTP 오류의 `state`와
`error_type`을 확인합니다.
...
합성 예외의 마스킹 검증용 문자열이 캡처에 남지 않았는지도 출력 없이 검사합니다. 다음
명령은 문자열이 없을 때만 종료 코드 0이어야 합니다.
...
저장소 밖의 OS 임시 디렉터리에 생성됩니다. 검증을 마치면 즉시 삭제합니다. 중간에
shell이 종료되어도 앞서 등록한 trap이 파일을 삭제합니다.
