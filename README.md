...
`ctr_model_training` DAG는 `schedule=None`인 수동 DAG로, `AUTORESEARCH_TRAINING_IMAGE`의
`src.cli train-model`을 실행하고 `MLFLOW_TRACKING_URI`(기본값
`http://mlflow.mlflow:5000`)를 주입해 MLflow tracking server(`mlflow` 네임스페이스,
`autoresearch-infra` 소관)에 Run이 기록되는지 검증합니다.

- 학습 이미지(`SKYAHO/Autoresearch`의 `Dockerfile.train`)는 아직 GAR push/digest
  승격 자동화가 없어 `AUTORESEARCH_TRAINING_IMAGE`를 수동으로 갱신해야 합니다.
- `airflow` → `mlflow` egress NetworkPolicy(`autoresearch-infra` 소관)가 적용되어
  있어야 Pod가 tracking server에 연결할 수 있습니다.
- 학습 데이터 소스(Feast/BigQuery 실 데이터 연동)는 이 DAG의 범위 밖이며 별도
  이슈에서 다룹니다.
