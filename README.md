...
feature 계층(`feast_offline_store`)의 Feast source 테이블을 재구축합니다.
...
- Dataset 트리거라 logical date 결합이 없습니다. 과거 파티션을
  `dag_run.conf.partition_date`로 수동 재적재하면 그 run의 검증이 성공하는 즉시
...
- 재구축 대상은 `user_static_feature`, `user_dynamic_feature`, `video_feature`
  세 테이블입니다. `user_category_similarity`는 원본 embedding artifact
  테이블(`user_topic_embedding`, `category_embedding`)을 적재하는 배치가 아직
  없어 제외되어 있으며, 기존 데이터를 그대로 둡니다.
- 적재는 테이블별 `TRUNCATE TABLE` + `INSERT INTO`이며 전체 재구축이라
  재실행해도 결과가 같습니다. `CREATE OR REPLACE`나 `WRITE_TRUNCATE`는 terraform
  (`autoresearch-infra`)이 소유한 대상 테이블 스키마를 query 결과 스키마로
  교체하므로 사용하지 않습니다.
- 적재 직후 테이블마다 비어있음·entity/timestamp NULL·중복 키를 SQL assertion으로
  검증하고, 위반 시 task를 실패시켜 downstream materialize를 막습니다.
