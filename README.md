...
- Dataset 트리거라 logical date 결합이 없습니다. 과거 파티션을
  ...
  feature build와 materialize가 다시 돕니다. ExternalTaskSensor 방식에서는
  같은 logical date의 upstream run만 인정해 이 재적재가 downstream에 반영되지
  않았습니다.
...
`feast_online_store_materialize` DAG도 매일 KST 00:00에 생성됩니다. 같은 logical
date의 `feast_offline_feature_build` run이 feature 테이블 재구축·검증을 모두
성공할 때까지 reschedule sensor로 대기한 뒤, Feast의
`autoresearch.jobs.feast_materialize`를 인자 없이 실행합니다. 따라서 대상 범위는
Airflow 날짜가 아니라 Feast registry watermark가 관리하는 증분 범위입니다.
