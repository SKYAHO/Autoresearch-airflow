...
feast_online_store_materialize                          (매일 KST 00:00 cron)
  apply_feature_registry                                (FeatureView 정의 → Feast registry)
...
실행되며, `apply_feature_registry`(`autoresearch.jobs.feast_apply`) →
`materialize_online_store`(`autoresearch.jobs.feast_materialize`) 순으로 두
...
- `FEAST_*` 변수는 BigQuery offline store, GCS registry/staging, Redis Cluster
  ...
  credential을 넣지 않습니다.
- FeatureView 정의 변경은 이 DAG의 `apply_feature_registry` task가 매 run마다
  registry에 반영하므로, 수동 `kubectl exec` 절차가 필요하지 않습니다. 다만 하루
  1회 cron이라 배포 직후 **최대 24시간의 registry 드리프트 창**이 남습니다. 즉시
  반영이 필요하면 DAG를 수동 트리거하십시오.
- apply는 코드에서 사라진 FeatureView를 registry에서 삭제하고 해당 view의 Redis
  데이터도 정리합니다. 이 변경 이후 "main 브랜치 코드 = registry 진실"이 매일
  무인으로 강제됩니다.
- apply가 실패하면 materialize는 실행되지 않습니다. 낡은 registry로 online store를
  덮어쓰지 않기 위한 의도된 동작이며, 그 경우 online store는 전날 상태로 남습니다.
...
5. scheduler/webserver rollout 후 Airflow CLI 준비를 최대 2분간 대기하고, 배포 digest,
   import error 0건, production 8-task topology, Feast materialize 1-task topology와
