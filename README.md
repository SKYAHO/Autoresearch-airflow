...
feast_online_store_materialize                          (Feast → Redis materialize, 매일 KST 00:00 cron)
```
...
실행되며, Feast의 `autoresearch.jobs.feast_materialize`를 인자 없이 실행합니다.
upstream 완료를 기다리지 않으므로 실행 시점의 offline store 상태까지만
동기화합니다. 대상 범위는 Airflow 날짜가 아니라 Feast registry watermark가
관리하는 증분 범위이므로, 그날 늦게 만들어진 feature도 다음 날 run이 누락 없이
가져갑니다.
...
- `FEAST_*` 변수는 BigQuery offline store, GCS registry/staging, Redis Cluster
  ...
  credential을 넣지 않습니다.
- FeatureView 정의 변경은 materialize 전에 별도 변경 절차에서 `feast apply`로
  registry에 반영해야 합니다. 이 DAG는 schema/registry 변경을 수행하지 않습니다.
