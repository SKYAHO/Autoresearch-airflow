...
promote-model 서브커맨드는 ctr_training과 같은 학습 이미지(deployment/Dockerfile.train,
autoresearch.cli)에 들어 있다 — 별도 이미지가 필요 없다(SKYAHO/Autoresearch#342/#343).
"""
...
# 그 이미지 안의 autoresearch.cli 서브커맨드이기 때문에 새 Variable을 만들 필요가 없다.
...
# autoresearch/model_training/config.yaml registry.* 기본값과 일치시킨다.
...
# 이동이 통과한다(SKYAHO/Autoresearch autoresearch/model_registry/registry.py
