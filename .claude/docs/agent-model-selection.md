...
- 빠른 탐색 결과를 Airflow DAG, Helm, 배포 변경의 근거로 사용하기 전에

이 저장소의 DAG delivery, Helm values, git-sync, Workload Identity 책임은
`CLAUDE.md`와 관련 운영 문서를 정본으로 사용합니다. 이 문서는 그 사실을
복제하지 않고, 작업 역할별 모델 선택만 다룹니다.
...
| Middle | 범위가 명확한 DAG·Helm·workflow·문서 구현, 검증 명령 작성, 일반 코드 또는 문서 리뷰 | Top이 정한 범위와 완료 조건 안에서 작업하고, 변경한 동작을 직접 검증합니다. |
...
- Luna는 high effort로 일상 구현 큐에 사용하고, 여러 파일을 오가거나
...
- DAG schedule·retry·timeout, Helm values, Workload Identity, 배포·권한·비용
  영향, 여러 저장소의 책임 변경은 Top 티어가 계획과 최종 판단을 맡습니다.
- 범위가 확정된 DAG·Helm·문서 변경과 그 검증은 Middle 티어에 맡길 수 있습니다.
- Fast 티어의 조사 결과는 Middle 이상이 원본 파일·로그·공식 문서로
  재확인한 뒤에만 변경 근거로 사용합니다.
- 작업 중 운영 영향이나 전제가 불명확해지면 중단하고 Top 티어로 승격해
  계획을 다시 세웁니다.

## Review and Verification

- 작성자와 독립된 리뷰어가 변경 목적, diff, 검증 결과를 확인합니다.
- 일반 변경은 Middle 이상으로 리뷰하고, Secret·Workload Identity·배포
  영향이 있으면 Top 티어 검토를 포함합니다.
- 모델 티어가 사람의 승인, Helm 검증, CI, 운영 검증을 대체하지 않습니다.
